...
export function usePlacementProgressSave({ report, selected, existingRow, updateProgress, reloadProgress, canvasRef }) {
  ...
  const handleSave = async () => {
    ...
    setSaveError(null)
    try {
      if (!existingRow?.id) throw new Error('Upload the day\'s .bkt file before saving the chart.')
      const blob = await new Promise((res) => canvasRef.current?.toBlob(res, 'image/png'))
      if (!blob) throw new Error('Could not export the chart image.')
      const file = new File(
        [blob],
        `placement_progress_${report.report_date}_${selected?.id ?? 'equipment'}.png`,
        { type: 'image/png' },
      )
      const uploadRes = await uploadAttachment({
        coreRecordId: existingRow.id,
        domain: PLACEMENT_PROGRESS_DOMAIN,
        file,
      })
